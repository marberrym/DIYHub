...
let getProject = (req, res) => {
  ...
  let comments = db.query(
    `SELECT comment, creation_date, diy_users.first_name AS name, diy_users.email AS email FROM diy_comments INNER JOIN diy_users ON diy_comments.user_id = diy_users.id WHERE diy_comments.project_id =${projectId}`
  )
