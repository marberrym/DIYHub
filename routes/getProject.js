...
let getProject = (req, res) => {
  ...
  let projectData = {};
  let project = db.one(
    `SELECT first_name, last_name, category_title, creation_date, project_title, feature_image_url, time_range, cost_range FROM diy_projects INNER JOIN diy_users ON diy_projects.user_id = diy_users.id INNER JOIN diy_categories ON diy_projects.category_id = diy_categories.id WHERE diy_projects.id=${projectId}`
  )
  let steps = db.query(
