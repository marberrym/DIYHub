...
let getProject = (req, res) => {
  ...
  let collaborators = db.query(
    `SELECT diy_users.id AS id, first_name, last_name FROM diy_users INNER JOIN diy_collaborators ON diy_users.id = diy_collaborators.user_id WHERE diy_collaborators.project_id=3 AND diy_collaborators.collab_status=2`
  )
